...
$(function(){
	...
	if(!config.options.fileRoot) {
		...
		$.ajax({
		    ...
		    //	alert(data);
		       ...
		       fileRoot = '/' + document.location.pathname.substring(1, document.location.pathname.lastIndexOf('/') + 1) + 'userfiles/'+data+'/';
	...
	if(config.upload.multiple) {
		...
		$('#upload').unbind().click(function() {
			alert("not done");
			$(".fancybox-iframe").attr("src", "http://localhost/reedemer/admin/public/directory/uploadform");					
		});
